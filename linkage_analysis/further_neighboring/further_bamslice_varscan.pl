...
my $ref = "$ENV{HOME}/work/GRCh38.d1.vd1.fa";
-e $ref or die "ERROR::not exist $ref\n";

my $token_path=`ls $ENV{HOME}/Dropbox/work/|grep 'gdc-user-token'`;
if(!$token_path){die "ERROR::token file not exitst!!";}
chomp $token_path;
$token_path="$ENV{HOME}/Dropbox/work/$token_path";
my $token=`cat $token_path`;

my %gene_conversion=();
open(CAND, "$ENV{HOME}/Dropbox/work/somatic_gene_conversion/revise/further_linked_analysis_candidate.tsv") or die  "ERROR::cannot open candidate_conversion_variants.tsv\n";
...
while(<CAND>){
		...
		my ($start,$end);
		if($line[$col{min_germ}] ne "NA"){
				my @minG=split(/:/,$line[$col{min_germ}]);
				$start=$minG[1]-500;
		}else{$start=$line[$col{start}];}
		if($line[$col{max_germ}] ne "NA"){
				my@maxG=split(/:/,$line[$col{max_germ}]);
				$end=$maxG[1]+500;
		}else{$end=$line[$col{start}];}
...
my %norm2tumor=();
open(TNID,"further_bam_check/sample_list/tumor_normal_sample_id.tsv") or die "ERROR::cannot tumor_normal_sample_id.tsv";
...
open(RES,"nkf -Lu further_bam_check/sample_list/response.tsv|") or die "ERROR::cannot open response.tsv\n";
